...
TEST(PDRUpdate, testAdd)
{
    ...
    uint32_t handle = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &handle),
        0);
    EXPECT_EQ(handle, 1u);
...
TEST(PDRRemoveByTerminus, testRemoveByTerminus)
{
    ...
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, NULL), 0);
    pldm_pdr_remove_pdrs_by_terminus_handle(repo, 1);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 0u);
    pldm_pdr_destroy(repo);

    repo = pldm_pdr_init();
    EXPECT_EQ(pldm_pdr_add(repo, data.data(), data.size(), true, 2, NULL),
              0);
    EXPECT_EQ(pldm_pdr_add(repo, data.data(), data.size(), true, 1, NULL),
              0);
    ...
    EXPECT_EQ(pldm_pdr_add(repo, data.data(), data.size(), true, 1, NULL),
              0);
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 2, NULL), 0);
    EXPECT_EQ(pldm_pdr_add(repo, data.data(), data.size(), true, 1, NULL),
              0);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 3u);
...
TEST(PDRUpdate, testRemove)
{
    ...
    handle = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), true, 1, &handle),
        0);
    EXPECT_EQ(handle, 4u);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 4u);
    pldm_pdr_destroy(repo);
...
TEST(PDRAccess, testFindByType)
{
    ...
    uint32_t first = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &first),
        0);
    hdr->type = 2;
    uint32_t second = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &second),
        0);
    hdr->type = 3;
    uint32_t third = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &third),
        0);
    hdr->type = 4;
    uint32_t fourth = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &fourth),
        0);
...
TEST(PDRUpdate, testFindLastInRange)
{
    ...
    uint32_t handle1 = 0;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &handle1),
        0);
    uint32_t handle2 = 23;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &handle2),
        0);
    uint32_t handle3 = 77;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), false, 1, &handle3),
        0);
    uint32_t handle4 = 16777325;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), true, 1, &handle4),
        0);
    uint32_t handle5 = 16777344;
    EXPECT_EQ(
        pldm_pdr_add(repo, data.data(), data.size(), true, 1, &handle5),
        0);
