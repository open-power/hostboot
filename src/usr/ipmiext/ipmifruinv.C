...
#define JEDEC_SIGNAL_LOADING_MASK            3
...
errlHndl_t isdimmIpmiFruInv::addSpdDetails(std::vector<uint8_t> &io_data)
{
    ...
    do {
            ...
            l_dieCount <= JEDEC_DIE_COUNT_MASK)
        {
            ...
            const uint32_t l_realDieCount =
              JEDEC_DIE_COUNT_MIN_VALUE + l_dieCount;

            // Calculate the Number of Package Ranks per DIMM
            // JEDEC Standard No. 21-C. Page 4.1.2.12 – 14
            // b00 - 1 package rank ... b11 - 4 package ranks. All others reserved.
            // For 3DS total Logical Ranks per DIMM =
            //      (Logical Ranks per DIMM) * (Die Count)
            // JEDEC Standard No. 21-C. Page 4.1.2.12 – 15
            const uint32_t l_realRanks =
              (JEDEC_RANKS_MIN_VALUE + l_ranks) * l_realDieCount;
            ...
            l_capacityGiB = (l_realDensity *
                             (l_realBusWidth / l_realDevWidth) *
                             l_realRanks) >> 10;
        }
