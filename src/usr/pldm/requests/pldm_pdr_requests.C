...
errlHndl_t sendGetNumericEffecterValueRequest(const effecter_id_t i_effecter_id, uint32_t& io_value, uint8_t& io_byte_data_size)
{
    errlHndl_t errl = nullptr;
    ...
    {
        PLDM_INF("sendGetNumericEffecterValueRequest: PLDM_ERROR_UNSUPPORTED_PLDM_CMD io_value=0x%X", INVALID_VALUE);
        break;
    }
    else
    {
        ...
        {
            io_byte_data_size = 1;
            io_value = l_present_value[0]; // take the present value position
        }
        ...
        {
            io_byte_data_size = 2;
            memcpy(&io_value, l_present_value, sizeof(uint16_t));
        }
        ...
        {
            io_byte_data_size = 4;
            memcpy(&io_value, l_present_value, sizeof(uint32_t));
        }
    }
    ...
    PLDM_EXIT("sendGetNumericEffecterValueRequest i_effecter_id=0x%X (%d) SEEDING with io_value=0x%X (%d) io_byte_data_size=%d",
                 i_effecter_id, i_effecter_id, io_value, io_value, io_byte_data_size);
...
errlHndl_t sendRebootCounterUpdateRequest()
{
    ...
    if (reboot_count != INVALID_VALUE) // only if its a valid value
    {
