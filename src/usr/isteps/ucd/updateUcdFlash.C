          ...
          UtilMem&                     i_image)
{
    ...
    for (size_t loop = 0; loop < MAX_LOOPS; ++loop)
    {
        ...
        {
            TRACFCOMP(g_trac_ucd,INFO_MRK
                "updateAllUcdFlashImages: Loop %d: Determine UCDs in system",
                loop);
        }
        else
        {
            TRACFCOMP(g_trac_ucd,INFO_MRK
                "updateAllUcdFlashImages: Loop %d: Update UCDs in system",
                loop);
        }
            ...
            if (loop == 0)
            {
                if (!(strcmp(deviceId,"UCD9090A")))
                {
                    ...
                    TRACFCOMP(g_trac_ucd,INFO_MRK
                        ...
                        "deviceId = %s. Use Gen4Pass2 Images (bpv=%d)",
                        TARGETING::get_huid(powerSequencer),
                        deviceId, backplane_version);

                    // Finding UCD9090A is all we need from loop 0,
                    // so break from power-sequencer loop
                    break;
                }
                else
                {
                    // Keep looping through UCDs to look for UCD9090A
                    continue;
                }
            }
