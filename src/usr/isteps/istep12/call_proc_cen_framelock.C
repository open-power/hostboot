...
void* call_proc_cen_framelock (void *io_pArgs)
{
    ...
    p9_cen_framelock_args l_args;
    l_args.channel_init_timeout = CHANNEL_INIT_TIMEOUT_14US;
    l_args.frtl_auto_not_manual = 1;
    ...
    TRACFCOMP(ISTEPS_TRACE::g_trac_isteps_trace, "call_proc_cen_framelock: %d DMIs found",
            l_dmiTargetList.size());

    for (const auto & l_dmi_target : l_dmiTargetList)
    {
        TRACFCOMP( ISTEPS_TRACE::g_trac_isteps_trace,
            "p9_cen_framelock HWP target HUID %.8x",
        ...
        if (l_pChildMembufList.size() == 1)
        {
            ...
            FAPI_INVOKE_HWP(l_err, p9_cen_framelock, l_fapi_dmi_target, l_fapi_membuf_target, l_args );

            //  process return code.
            if ( l_err )
            {
                TRACFCOMP( ISTEPS_TRACE::g_trac_isteps_trace,
                    "ERROR 0x%.8X:  p9_cen_framelock HWP on target HUID %.8x",
            ...
            {
                TRACFCOMP( ISTEPS_TRACE::g_trac_isteps_trace,
                         "SUCCESS :  p9_cen_framelock HWP");
            }
        }
        else    //No associated membuf
        {
            TRACFCOMP( ISTEPS_TRACE::g_trac_isteps_trace,
                      "call_proc_cen_framelock HWP skipped, no associated membufs %d"
