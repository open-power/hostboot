...
/*                                                                        */
/* $Source: src/usr/diag/prdf/plat/odyssey/prdfOdysseyPlugins_rt.C $      */
/*                                                                        */
                        ...
                        STEP_CODE_DATA_STRUCT & io_sc, uint8_t i_port )
{
    ...
    else
    {
        ...
        if (getOcmbDataBundle(i_chip)->iv_rcdParityTh.thReached(io_sc))
        {
            ...
            getSlaveRanks<TYPE_OCMB_CHIP>( i_chip->getTrgt(), i_port, list );
...
PRDF_PLUGIN_DEFINE(odyssey_ocmb, RcdParityError);
