                   ...
                   uint64_t i_address)
{
    ...
    if ( nullptr != errl )
    {
        ...
        // operation.
        if (errl && errl->hasErrorType(MMIO::RC_MMIO_CHAN_CHECKSTOP))
        {
...
uint32_t getMemAddrRange( ExtensibleChip * i_chip, VT & o_startAddr,
                          VT & o_endAddr, const uint8_t& i_port,
                          uint8_t i_dimmSlct )
{
    ...
    do
    {
        ...
        o_rc = getMemAddrRange<TT>( i_chip, firstRank, i_port, o_startAddr,
                                    o_endAddr, MASTER_RANK );
        ...
        VT junk;
        o_rc = getMemAddrRange<TT>( i_chip, lastRank, i_port, junk, o_endAddr,
                                    MASTER_RANK );
        if ( SUCCESS != o_rc ) break;
...
void odyEnableEdpl(TARGETING::TargetHandle_t i_ocmb)
{
    PRDF_ASSERT(nullptr != i_ocmb);
    PRDF_ASSERT(TYPE_OCMB_CHIP == getTargetType(i_ocmb));
    PRDF_ASSERT(isOdysseyOcmb(i_ocmb));
    ...
    FAPI_INVOKE_HWP(errl, ody_omi_edpl, fapiOcmb, true);
    if (nullptr != errl)
    {
        PRDF_ERR("ody_omi_edpl(0x%08x) failed", getHuid(i_ocmb));
        PRDF_COMMIT_ERRL(errl, ERRL_ACTION_REPORT);
    }
}
