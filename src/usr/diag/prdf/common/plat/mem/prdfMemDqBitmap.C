...
uint32_t MemDqBitmap::setDq( uint8_t i_dq )
{
...
uint32_t MemDqBitmap::setSymbol( const MemSymbol & i_symbol, uint8_t i_pins )
{
    ...
    do
    {
        ...
        o_rc = getByteBitIdx( i_symbol, byteIdx, bitIdx );
        if ( SUCCESS != o_rc )
        {
            PRDF_ERR( PRDF_FUNC "getByteBitIdx() failed" );
            break;
        }

        uint32_t shift = (DQS_PER_BYTE-1) - bitIdx;

        // The number of DQs per symbol is 1 for DIMMs with x4 drams and 2 for
        // DIMMs with x8 drams.
        size_t dqsPerSym = 2;
        if (iv_x4Dram)
        {
        ...
        iv_bitmap[byteIdx] |= i_pins << shift;
...
{
    memset( iv_bitmap, 0x00, sizeof(iv_bitmap) );
}
...
void MemDqBitmap::getCaptureData( CaptureData & o_cd ) const
{
    ...
    size_t sz_capData = sz_rank + sizeof(iv_bitmap);
    ...
    uint8_t idx = 1;
    memcpy( &capData[idx], iv_bitmap, sizeof(capData[idx]) );
...
uint32_t MemDqBitmap::getByteBitIdx( const MemSymbol & i_symbol,
                                     uint8_t & o_byteIdx,
                                     uint8_t & o_bitIdx ) const
{
    #define PRDF_FUNC "[MemDqBitmap::getByteBitIdx] "
...
uint32_t MemDqBitmap::isChipMark( const MemSymbol & i_symbol, bool & o_cm )
{
    ...
    do
    {
        ...
        uint8_t cmData = iv_bitmap[byteIdx];
                                    ...
                                    bool & o_symOnSp1 )
{
    ...
    // Get the spares
    MemSymbol sp0, sp1;
    rc = mssGetSteerMux<TYPE_MEM_PORT>( iv_trgt, iv_rank, sp0, sp1 );
    if ( SUCCESS != rc )
    {
...
std::vector<MemSymbol> MemDqBitmap::getSymbolList()
{
    ...
    uint8_t symbolsPerByte = SYMBOLS_PER_RANK/(BYTES_PER_DIMM*dqsPerSym);

    // Calculate bit mask -> (2^dqsPerSym)-1
    uint8_t bitMask = 2;
    for ( uint8_t i = 1; i < dqsPerSym; i++ ) {bitMask *= 2;}
    bitMask -= 1;
    ...
    for ( uint8_t byte = 0; byte < DQ_BITMAP::BITMAP_SIZE; byte++ )
    {
        ...
        {
            uint8_t shift = ((symbolsPerByte - 1) - symIdx) * dqsPerSym;

            // if the bit/bit pair is active
            if ( ((iv_bitmap[byte] >> shift) & bitMask) != 0 )
            {
                // get the dq
                uint8_t dq = (byte * DQS_PER_BYTE)+(symIdx * dqsPerSym);
                ...
                if ( onSp0 || onSp1 )
                {
                    ...
                    // bits set there.
                    uint8_t spByte = iv_bitmap[iv_spareByte];
                ...
                o_symbolList.push_back( MemSymbol::fromSymbol(iv_trgt, iv_rank,
                                                              symbol) );
            }
        }
    }
...
uint32_t __getSpareInfo( TargetHandle_t i_trgt, MemRank i_rank, bool i_x4Dram,
    uint8_t & o_spareConfig, uint8_t & o_noSpare, uint8_t & o_lowNibble,
    uint8_t & o_highNibble, bool & o_spareSupported )
{
    ...
    uint8_t ps = i_trgt->getAttr<TARGETING::ATTR_REL_POS>();
    o_rc = getDimmSpareConfig<TYPE_MEM_PORT>( i_trgt, i_rank, ps,
                                              o_spareConfig );
    if( SUCCESS != o_rc )
    {
...
uint32_t MemDqBitmap::isSpareAvailable( bool & o_sp0Avail, bool & o_sp1Avail )
{
    ...
    do
    {
        ...
        o_rc = __getSpareInfo( iv_trgt, iv_rank, iv_x4Dram, spareConfig,
        ...
        uint8_t spareDqBits = iv_bitmap[iv_spareByte];
...
template<>
uint32_t setDramInVpd<TYPE_MEM_PORT>( TargetHandle_t i_trgt,
...
template<>
uint32_t clearDramInVpd<TYPE_MEM_PORT>( TargetHandle_t i_trgt,
    const MemRank & i_rank, MemSymbol i_symbol )
{
    #define PRDF_FUNC "[MemDqBitmap::clearDramInVpd] "
