...
{
    #define PRDF_FUNC "[MemUtils::hwfmSetExit1] "

    uint32_t o_rc = SUCCESS;

    // Not needed for Explorer as HWFM is not triggered on NCEs for Explorer
    if (!isOdysseyOcmb(i_ocmb->getTrgt()))
    {
        return o_rc;
    }
    ...
    // Set RECR[9:10] = 0b01 to set MIN_EXIT_1 preventing HWFM for NCEs.
    recr->ClearBit(9);
    recr->SetBit(10);
    ...
    if (SUCCESS != o_rc)
    {
        PRDF_ERR(PRDF_FUNC "Failed to Write RECR on 0x%08x", i_ocmb->getHuid());
        return o_rc;
    }

    // Wait 1 second to allow any read in flight using exit 0 to complete
    milliSleep(1,0);
