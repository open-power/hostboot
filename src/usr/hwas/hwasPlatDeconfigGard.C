...
errlHndl_t _GardRecordIdSetup( void *&io_platDeconfigGard)
{
    ...
    do
    {
        ...
        switch (l_pGardRecordsBinary->iv_version)
        {
            ...
            // This handles the case where a system was upgraded
            case(HWAS::DeconfigGard::GARD_VERSION_P10_GA):
                {
                HWAS_INF("_GardRecordIdSetup: CASE DIMM Migration Forward: l_hbDeconfigGard->iv_GardVersion=0x%X",
                    l_hbDeconfigGard->iv_GardVersion);
                std::vector<DeconfigGard::GardRecordPair_t> l_changes;
                l_pErr = theDeconfigGard().migrateDimmGardRecordsForward( l_changes );
                if( l_pErr )
                {
