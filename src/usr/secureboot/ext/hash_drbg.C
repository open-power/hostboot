                                ...
                                size_t i_requested_bytes)
{
    ...
    do
    {
        ...
        SHA512_t H;
        SECUREBOOT::hashBlob(h_input, sizeof(h_input), H);
                        ...
                        size_t i_output_length)
{
    ...
    for (uint8_t counter = 1; counter <= len; counter++)
    {
        ...
        *reinterpret_cast<uint32_t*>(hash_input + 1) = no_of_bits_to_return;
        ...
        SECUREBOOT::hashBlob(hash_input, hash_input_len, hash_output);
                        ...
                        uint8_t* o_output)
{
    ...
    for (size_t i = 1; i <= m; i++)
    {
        // w = Hash(data)
        SHA512_t w;
        SECUREBOOT::hashBlob(data, HASH_DRBG_SEEDLEN_BYTES, w);
