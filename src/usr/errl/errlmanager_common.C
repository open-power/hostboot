...
void ErrlManager::handleSpareCoreErrors(errlHndl_t & io_err)
{
    const char* l_sevString = errl_sev_str_map.at(io_err->sev());
    TRACFCOMP(g_trac_errl, ENTER_MRK"handleSpareCoreErrors() eid=%.8x, Reasoncode=%.4X, Sev=%s",
    ...
    {
        static_assert(HWAS::LAST_CALLOUT == HWAS::VRM_CALLOUT, "New callout added, handleSpareCoreErrors needs update");
        for (const auto ud_callout : io_err->getUDSections(ERRL_COMP_ID, ERRORLOG::ERRL_UDT_CALLOUT))
        {
            ...
            if (calloutHasServiceActions && HWAS::theDeconfigGard().reduceSpareCores(target))
            {
                ...
                TRACFCOMP(g_trac_errl,
                          "handleSpareCoreErrors(): eid=%.8X RC=%.4X and Sev=%s is a SPARE error",
                          io_err->eid(), io_err->reasonCode(), l_sevString);
    ...
    {
        TRACFCOMP(g_trac_errl,
                  "handleSpareCoreErrors(): Spareable error detected, reducing severity from %s to RECOVERED",
                   l_sevString);
