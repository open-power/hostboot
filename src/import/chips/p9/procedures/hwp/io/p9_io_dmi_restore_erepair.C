...
/*                                                                        */
/* $Source: src/import/chips/p9/procedures/hwp/io/p9_io_dmi_restore_erepair.C $ */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file p9_io_dmi_restore_erepair.C
...
fapi2::ReturnCode set_rx_bad_lane_vectors(
    const fapi2::Target< fapi2::TARGET_TYPE_DMI >& i_rx_target,
    ...
    const std::vector< uint8_t >&                   i_bad_lanes)
{
    ...
    for(uint8_t index = 0; index < i_bad_lanes.size(); ++index)
    {
        ...
        else
        {
            ...
            FAPI_TRY(io::read(EDIP_RX_LANE_BAD_VEC_16_23, i_target, GRP3, LN0, l_data),
                     "RMW rx_bad_lane_16_23_reg failed");
            l_data |= (0x8000 >> bad_lane);
            FAPI_TRY(io::write(EDIP_RX_LANE_BAD_VEC_16_23, i_target, GRP3, LN0, l_data),
                     "RMW rx_bad_lane_16_23_reg failed");
        }
