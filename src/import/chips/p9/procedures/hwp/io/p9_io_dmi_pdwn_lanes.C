...
/*                                                                        */
/* $Source: src/import/chips/p9/procedures/hwp/io/p9_io_dmi_pdwn_lanes.C $ */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file p9_io_dmi_pdwn_lanes.C
/// @brief P9 DMI Power Down Lanes.
///----------------------------------------------------------------------------
...
/**
 * @brief A HWP that powers down the specified lanes on a given EDI+ DMI
 ...
 * @retval ReturnCode
 */
fapi2::ReturnCode p9_io_dmi_pdwn_lanes(
    const fapi2::Target< fapi2::TARGET_TYPE_DMI >& i_target,
    ...
    const std::vector< uint8_t >&                   i_bad_lanes)
{
    ...
    if(!i_bad_lanes.empty())
    {
        for(uint8_t index = 0; index < i_bad_lanes.size(); ++index)
        {
            ...
            FAPI_TRY(io::rmw(EDIP_RX_LANE_DIG_PDWN, i_target, GRP3, i_bad_lanes[index], 1),
                     "Failed rmw rx dig pdwn reg");

            FAPI_TRY(io::rmw(EDIP_RX_LANE_ANA_PDWN, i_target, GRP3, i_bad_lanes[index], 1),
    ...
    const std::vector< uint8_t >&                   i_bad_lanes)
{
    ...
    if(!i_bad_lanes.empty())
    {
        FAPI_TRY(io::read(EDIP_TX_MSBSWAP, i_target, GRP3, LN0, l_data),
                 "Failed read edip_tx_msbswap");
        l_msbswap = io::get(EDIP_TX_MSBSWAP, l_data);

        if(l_msbswap == 0x1)
        {
            FAPI_TRY(io::read(EDIP_TX_END_LANE_ID, i_target, GRP3, LN0, l_data),
        ...
        for(uint8_t index = 0; index < i_bad_lanes.size(); ++index)
        {
            ...
            FAPI_TRY(io::rmw(EDIP_TX_LANE_PDWN, i_target, GRP3, l_lane, 1),
                     "Failed rmw tx pdwn");
