                                 ...
                                 const uint8_t i_abort_on_error ) const
{
    ...
    for (const auto& l_rank : l_ranks)
    {
        ...
        // 3) Put the buffer into MWD training mode - host issues BCW commands
        ...
        for(uint8_t l_delay = 0; l_delay < MWD_MAX_DELAY; l_delay++)
        {
            ...
            // 4) Sets the delay across all buffers, all nibbles - host issues BCW commands to buffers (cw_info)
            ...
            // 6) add in NTTM mode read here - >forces the logic to read out the data
        ...
        // 8) Takes the buffer out of MWD and sets it into mainline mode
        FAPI_TRY(set_buffer_training(l_dimm, ddr4::NORMAL), "%s failed set_buffer_training", mss::c_str(l_dimm));

        // 9) Finds the best delay for each bit
        ...
        FAPI_TRY( find_best_delay_for_each_dq(i_target, l_rank, l_recorder, l_eye_sizes_dq, l_final_nibble_delays_buffer),
                  ...
                  l_rank);
        // 10) Set best case results into buffer using PBA
