...
#
# $Source: src/import/chips/p10/procedures/ppe/fw_tools/udparsers/b2e00/b2e00.py $
#
...
# @brief    finds start of PGPE trace buffer
# @param    content of error log
def parseXgpeUserDataSection(data):

    with open( "/tmp/pgpe.bin", "wb" ) as pgpeTraceBinFile:
...
def parsePpeRegSection( pgpeReg ):
    pgpeReg.seek( ERRL_BIN_XIR_OFFSET )
    pgpe_xir_val = "[\n"
    pgpe_xir_val += "XCR-CTR    :  " + '{:016x}'.format( int.from_bytes(pgpeReg.read( 8 ), byteorder='big' )) + "\n"
    pgpe_xir_val += "XSR-SPRG0  :  " + '{:016x}'.format( int.from_bytes(pgpeReg.read( 8 ), byteorder='big' )) + "\n"
    pgpe_xir_val += "IR-EDR     :  " + '{:016x}'.format( int.from_bytes(pgpeReg.read( 8 ), byteorder='big' )) + "\n"
    pgpe_xir_val += "XSR-IAR    :  " + '{:016x}'.format( int.from_bytes(pgpeReg.read( 8 ), byteorder='big' )) + "\n"
    pgpe_xir_val += "SRR0-LR    :  " + '{:016x}'.format( int.from_bytes(pgpeReg.read( 8 ), byteorder='big' )) + "\n"
    pgpe_xir_val += "]\n"
    pgpeReg.seek( 0 )
    return pgpe_xir_val
...
def parseUDToJson(subType, ver, data):
    ...
    while size_parsed < ELOG_SIZE  and l_sectn < MAX_UD_SECTN :
        ...
        pgpeReg.seek( size_parsed )
        udType = int.from_bytes( pgpeReg.read( 1 ), byteorder='big' )
        udSize = int.from_bytes( pgpeReg.read( 2 ), byteorder='big' )
        udSize += 8
        size_parsed += udSize

        if udType == elog_sectn_list.get( "XIR_REG" ) :
            pgpe_xir_reg = parsePpeRegSection( pgpeReg )
            traceDict["XIRs"] = pgpe_xir_reg.split( "\n" )
    ...
    if os.path.exists( "/tmp/pgpe.bin" ):
        os.remove( "/tmp/pgpe.bin" )
