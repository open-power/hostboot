    ...
    const uint16_t i_vendor_id)
{
    ...
    FAPI_INF("Setting PMIC " GENTARGTIDFORMAT " settings from SPD", GENTARGTID(i_pmic_target));
    ...
    FAPI_ASSERT((i_vendor_id == mss::pmic::vendor::IDT ||
                ...
                "Unknown PMIC: " GENTARGTIDFORMAT " with vendor ID 0x%04hhX",
    ...
    else
    {
        FAPI_TRY(mss::pmic::bias_with_spd_settings<mss::pmic::vendor::TI>(i_pmic_target, i_ocmb_target, l_relative_pmic_id),
                 "enable_spd (TI): Error biasing PMIC " GENTARGTIDFORMAT " with SPD settings",
                 GENTARGTID(i_pmic_target));
    }

    // Start VR Enable
    FAPI_TRY(mss::pmic::start_vr_enable(i_pmic_target),
             "Error starting VR_ENABLE on PMIC " GENTARGTIDFORMAT, GENTARGTID(i_pmic_target));
