...
/*                                                                        */
/* $Source: src/import/chips/ocmb/odyssey/procedures/hwp/io/common/common_io_tdr.C $ */
/*                                                                        */
    ...
    const uint32_t& i_freq)
{
    ...
    for (int32_t l_phase = 0; l_phase <= 1; l_phase++)
    {
        ...
        switch (l_status[l_phase])
        {
            ...
            case TdrResult::Good:
                {
                    FAPI_DBG("TDR Found a good net for Lane(%d) and Phase(%d)", i_lane, l_phase);
                }
        }
    ...
    if ((o_status == TdrResult::Good) || (o_status == TdrResult::UnableToDetermine))
    {
        o_length_ps = 0;
    }
    else
...
/// @param[in] i_target             IOLINK target
/// @param[in] i_pw                 TDR pulse width
