...
///
/// @brief Check and return port position from sPPR chip-op input data
/// @param[in] i_target ocmb target
/// @param[in] i_ports vector of ports in the given ocmb target
/// @param[in] i_repair_buf the repair data
/// @param[out] o_port_pos the port position of the requested sPPR
/// @return FAPI2_RC_SUCCESS iff successful
///
fapi2::ReturnCode find_port_pos_from_sppr_input(
    const fapi2::Target<fapi2::TARGET_TYPE_OCMB_CHIP>& i_target,
    ...
    uint8_t& o_port_pos )
{
    ...
    const uint8_t l_ports_on_ocmb = i_ports.size();
    bool l_found_port_pos = false;

    const uint8_t l_desired_port_pos = i_repair_buf.getBit<TT::ROW_REPAIR_CHIPOP_PORT>();

    for (o_port_pos = 0; o_port_pos < l_ports_on_ocmb; o_port_pos++)
    {
        if (l_desired_port_pos == mss::relative_pos<mss::mc_type::ODYSSEY, fapi2::TARGET_TYPE_OCMB_CHIP>(i_ports[o_port_pos]))
        {
            l_found_port_pos = true;
            break;
        }
    }

    // Fail if we didn't find our port position
    FAPI_ASSERT(l_found_port_pos,
                fapi2::ODY_PORT_INDEX_OUT_OF_BOUNDS().
                set_OCMB_TARGET(i_target).
                set_PORT_INDEX(l_desired_port_pos).
                set_PORTS_ON_OCMB_CHIP(l_ports_on_ocmb),
                "Port index %d supplied to sppe_dynamic_row_repair is out of bounds on " TARGTIDFORMAT,
                l_desired_port_pos, TARGTID);
    ...
    const fapi2::buffer<uint64_t>& i_repair_buf )
{
    ...
    FAPI_TRY(find_port_pos_from_sppr_input(i_target, l_ports, i_repair_buf, l_port_pos));

    {
        ...
        return dynamic_row_repair(l_rank_info, l_repair);
    }
