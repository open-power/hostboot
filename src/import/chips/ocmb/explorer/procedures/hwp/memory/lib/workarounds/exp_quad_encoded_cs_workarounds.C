...
fapi2::ReturnCode fix_mirroring_bitmap_attribute(const fapi2::Target<fapi2::TARGET_TYPE_DIMM>& i_target)
{
    ...
    uint8_t l_mirroring_attr = 0;

    // Read
    FAPI_TRY(mss::attr::get_exp_dram_address_mirroring(i_target, l_mirroring_attr));

    // Modify (well potentially)
    FAPI_TRY(mss::is_quad_encoded_cs(i_target, l_is_quad_encoded_cs));
    fix_mirroring_bitmap_for_quad_encoded_cs(l_is_quad_encoded_cs, l_mirroring_attr);

    // Write
    FAPI_TRY(mss::attr::set_exp_dram_address_mirroring(i_target, l_mirroring_attr));
