...
///
template<>
fapi2::ReturnCode  configure_bank<mss::mc_type::EXPLORER>(const fapi2::Target<fapi2::TARGET_TYPE_MEM_PORT>& i_target,
        uint64_t& io_current_bit_index,
        mcbist::program<mss::mc_type::EXPLORER>& io_program)
{
    uint8_t l_attr_bank[2] = {};

    // There are 2 bank address: BA0, BA1
...
///
template<>
fapi2::ReturnCode configure_bank_groups<mss::mc_type::EXPLORER>(const fapi2::Target<fapi2::TARGET_TYPE_MEM_PORT>&
        i_target,
        uint64_t& io_current_bit_index,
        mcbist::program<mss::mc_type::EXPLORER>& io_program)
{
    ...
    // There are 2 bank groups address: BG0, BG1
    ...
    // There is some endiness difference b/w the IBM appdress map and JEDEC bit mapping, hence the mapping are swapped
    io_program.change_bank_group0_bit(l_attr_bg_bits[0] > 1 ? io_current_bit_index-- : 0);
    io_program.change_bank_group1_bit(l_attr_bg_bits[0] > 0 ? io_current_bit_index-- : 0);
