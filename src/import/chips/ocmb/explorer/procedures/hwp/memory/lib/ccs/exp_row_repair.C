        ...
        std::vector<mss::row_repair::repair_entry<mss::mc_type::EXPLORER>>& o_repairs_per_dimm)
{
    ...
    for (uint8_t l_dimm_rank = 0; l_dimm_rank < l_kind.iv_master_ranks; ++l_dimm_rank)
    {
        ...
        if (l_entry.is_valid())
        {
            ...
            FAPI_DBG("Maxes for dimm %s: DRAM %d, srank %d, bg %d, bank %d, row 0x%05x",
                     mss::spd::c_str(i_target), l_num_dram, l_num_subrank, MAX_BANK_GROUP, MAX_BANKS, l_kind.iv_rows);
