...
/*                                                                        */
/* $Source: src/import/chips/centaur/procedures/hwp/memory/wrapper/p9c_mss_memdiags_wrap.C $ */
/*                                                                        */
...
/// @file p9c_mss_memdiags_wrap.C
/// @brief Wrapper for memdiags procedure
///
...
{
    // procedure constants
    std::string PROCEDURE = "p9c_mss_memdiags_wrap";
...
{
    // procedure constants
    const std::string PROCEDURE = "mss_memdiags_wrap";
    ...
    do
    {
        ...
        while (ecmdConfigLooperNext(node_target, node_looper))
        {
            ...
            while(ecmdConfigLooperNext(cen_target, cen_looper))
            {
                ...
                fapi2::Target<fapi2::TARGET_TYPE_MBA>  fapi2_target(&cen_target);
                FAPI_EXEC_HWP(rc_fapi2,
                              p9c_mss_memdiags,
                              fapi2_target);
                rc = (uint32_t) rc_fapi2;
