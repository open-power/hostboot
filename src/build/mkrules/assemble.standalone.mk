...
# WOF input images
$(info    WOF_ARTIFACT_LOCATION is $(WOF_ARTIFACT_LOCATION))
WOFDATA_IMG := ${call get_files_full_path, ${WOF_ARTIFACT_LOCATION}}

# DEVTREE input image
$(info    DEVTREE_ARTIFACT_ID is $(DEVTREE_ARTIFACT_ID))
DEVTREE_IMG := ${DEVTREE_ARTIFACT_ID}

# OCC input image
$(info    OCC_ARTIFACT_ID is $(OCC_ARTIFACT_ID))
OCC_IMG := ${OCC_ARTIFACT_ID}
...
# NOTE: For default standalone PNOR layout HB_HLL will get called with a later script,
#       so that is why it's not in the following list
DEF_GEN_BIN_FILES := HBBL=${HBBL_IMG},HBB=${HBB_IMG},HBI=${HBI_IMG},\
	HBRT=${HBRT_IMG},HBEL=EMPTY,GUARD=EMPTY,RINGOVD=EMPTY,\
...
ifeq (${HBD_RW_EXISTS},)
    ...
    HBD=${HBD_COMBO_IMG},WOFDATA=${WOFDATA_IMG},DEVTREE=${DEVTREE_IMG},HCODE_LID=${HCODE_LID_IMG},PSPD=${PSPD_FINAL_IMG},HB_HLL=EMPTY
else
    _GEN_BIN_FILES := SBE=${SBE_IMG},HCODE=${HCODE_IMG},OCC=${OCC_IMG},\
    HBD=${HBD_COMBO_IMG},HBD_RW=${HBD_RW_FINAL_IMG},WOFDATA=${WOFDATA_IMG},DEVTREE=${DEVTREE_IMG},HCODE_LID=${HCODE_LID_IMG},PSPD=${PSPD_FINAL_IMG},HB_HLL=EMPTY
endif
...
# If you do NOT export BMC_STANDALONE_PATCHING you get the normal STANDALONE
ifeq (${BMC_STANDALONE_PATCHING},)
# For standalone PNOR layout
	...
	HB_HLL=${HB_HLL_FINAL_IMG}
else
# For BMC standalone PNOR layout
    HOSTBOOT_DEFAULT_SECTIONS := HBBL=${HBBL_FINAL_IMG} HBB=${HBB_FINAL_IMG} \
	HBI=${HBI_FINAL_IMG} HBRT=${HBRT_FINAL_IMG} \
	HBEL=${HBEL_FINAL_IMG} GUARD=${GUARD_FINAL_IMG} \
	PAYLOAD=${PAYLOAD_FINAL_IMG}  \
	RINGOVD=${RINGOVD_FINAL_IMG} SBKT=${SBKT_FINAL_IMG} \
	VERSION=${VERSION_FINAL_IMG} PSPD=${PSPD_FINAL_IMG} \
...
ifeq (${HBD_RW_EXISTS},)
    SECT := HBD=${HBD_FINAL_IMG} SBE=${SBE_FINAL_IMG} HCODE=${HCODE_FINAL_IMG} \
        ...
        HCODE_LID=${HCODE_LID_FINAL_IMG} PSPD=${PSPD_FINAL_IMG}
else
    SECT := HBD=${HBD_FINAL_IMG} HBD_RW=${HBD_RW_FINAL_IMG} \
        SBE=${SBE_FINAL_IMG} HCODE=${HCODE_FINAL_IMG} \
        OCC=${OCC_FINAL_IMG} WOFDATA=${WOFDATA_FINAL_IMG} \
        EECACHE=${EECACHE_FINAL_IMG} \
        OCMBFW=${OCMBFW_FINAL_IMG} DEVTREE=${DEVTREE_FINAL_IMG} \
        HCODE_LID=${HCODE_LID_FINAL_IMG} PSPD=${PSPD_FINAL_IMG}
